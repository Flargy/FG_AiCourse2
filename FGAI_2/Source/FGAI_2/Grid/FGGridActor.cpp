...
AFGGridActor::AFGGridActor()
{
	...
	LineBatch = CreateDefaultSubobject<ULineBatchComponent>(TEXT("LineBatchComponent"));
...
void AFGGridActor::DrawBlocks()
{
	...
	for (int32 Y = Height - 1; Y >= 0; --Y)
	{
		for (int32 X = 0; X < Width; ++X)
		{
			FVector TileWorldLocation = GetWorldLocationFromXY(X, Y);
			
			const int32 ArrayIndex = GetTileIndexFromXY(X, Y);

			const bool bIsBlocked = TileList[ArrayIndex].bBlock;
			FColor Color = bIsBlocked ? FColor::Red : FColor::Green;
			Color.A = 100;
			FBox Box = FBox::BuildAABB(TileWorldLocation, BlockExtent);
			LineBatch->DrawSolidBox(Box, FTransform::Identity, Color, 0, 0.0f);
		}
	}
}
